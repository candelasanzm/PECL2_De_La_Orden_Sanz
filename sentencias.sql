-- Sentencia 1
select final_colisionVehiculos.vehicle_id, count(final_colisionVehiculos.collision_id) as NumeroAccidentes
from final_colisionVehiculos
group by vehicle_id
having count(final_colisionVehiculos.collision_id) > 1
...
-- Sentencia 4
select count(final_colisionPersona.collision_id) as NumeroAccidentes, final_colisionPersona.person_id
from final_colisionPersona
group by person_id
having count(final_colisionPersona.collision_id) > 1
order by NumeroAccidentes desc;
...
from final_persona, final_vehiculos
where final_vehiculos.vehicle_type = 'Pick-up';

-- Sentencia 7
select count(cv.unique_id) as NumeroAccidentes, v.vehicle_make
from final_colisionVehiculos as cv left join final_vehiculos as v on cv.vehicle_id = v.vehicle_id
...
from final_persona, final_accidentes, final_colisionPersona
